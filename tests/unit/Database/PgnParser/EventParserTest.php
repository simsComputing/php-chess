...
use Chess\Database\PgnParser\EventParser;
...
class EventParserTest extends TestCase
{
    ...
    public function testInfoOnPgnParser($pgnString, $expected)
    {
        ...
        $this->pgnParser->expects($this->once())->method('setEvent')->with($expected);

        foreach (str_split($pgnString) as $char) {
            if (InfoParserResult::DONE === $eventParser->supports($char)) {
                $eventParser->setInfoOnPgnParser($this->pgnParser);
                break;
            }
        }
    }
    ...
    {
        return [
            ['[Event "Championnat du monde"]', "Championnat du monde"],
            [<<<PGN
